...
def convert_files():
    ...
    main_path  = folder_path+"/"+app.config["ROOT_DIR_NAME"]+"/"
    Path(main_path).mkdir(parents=True, exist_ok=True)
    
    convert_path = main_path+"/"+app.config["OUTPUT_DIR_NAME"]+"/"
    Path(convert_path).mkdir(parents=True, exist_ok=True)
    
    # rename files first
    for item_name in os.listdir(main_path):
        rename_file_and_remove_whitespace(main_path+item_name)
       
    #convert files 
    for item_name in os.listdir(main_path):
        full_path = os.path.join(main_path, item_name)
        if os.path.isfile(full_path):
            new_name = re.sub(r'\.[^.]*$', '', item_name)
            convert_mp4_to_mp3(main_path+item_name, convert_path+new_name+".mp3")
